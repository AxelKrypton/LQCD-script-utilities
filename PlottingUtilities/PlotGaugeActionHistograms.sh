...
BETA_ARRAY=()
DIR_WITH_INPUT_FILES=$(ls | grep "mui.\{1,3\}_k[[:digit:]]\{4\}_nt[[:digit:]]\{1,2\}_ns[[:digit:]]\{1,2\}_reweighting")
if [ ! -d $DIR_WITH_INPUT_FILES ]
then
...
then
    ...
    do
        if [ -f "$DIR_WITH_INPUT_FILES/rawDataForReweighting_b$BETA" ]
        then
...
if [ ${#INPUT_FILES_ARRAY[@]} -eq 0 ]
then
    echo "No input files found...exiting"
    exit
fi

for INDEX in ${!INPUT_FILES_ARRAY[@]}
do
    add_plot
done
